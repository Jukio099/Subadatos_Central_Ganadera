...
def obtener_links_pdf_de_pagina(url: str) -> list[dict]:
    ...
    for a_tag in soup.find_all("a", href=True):
        ...
        if href.endswith(".pdf") and "wp-content/uploads" in href:
            ...
            match_anio = re.search(r"/uploads/(\d{4})/", href)
            ...
            if match_fecha:
                ...
                match_fecha_no_year = re.search(r"[_\-](\d{1,2})[_\s\-]*de[_\s\-]*([a-zñáéíóú]{3,})", href, re.IGNORECASE)
                # Format 4: DD_MM without year
                match_fecha_num_no_year = re.search(r"[_\-](\d{1,2})[_\-](\d{1,2})(?:_|$|\.)", href)
            ...
            if fecha is None:
                print(f"  ⚠️  No se pudo extraer fecha del archivo: {os.path.basename(href)}")
